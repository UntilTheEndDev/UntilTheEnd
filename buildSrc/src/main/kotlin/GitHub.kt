...
object GitHub {
    ...
    private fun getGithubToken(project: Project): String {
        ...
        System.getProperty("github_token", null)?.let {
            return@getGithubToken it.trim()
        }
        System.getenv("github_token")?.let { return@getGithubToken it.trim() }
    ...
    fun upload(file: File, url: String, project: Project) = runBlocking {
        ...
        Http.put<String>("$url?access_token=$token") {
            val sha = getGithubSha("UntilTheEndReleases", "shadow/${project.name}/${file.name}", "master", project)
            println("sha=$sha")
        ...
        * */
        ...
        suspend fun getShaSmart(repo: String, filePath: String, project: Project): String? {
            return withContext(Dispatchers.IO) {
                val response = Jsoup
                        .connect(
                                "https://api.github.com/repos/UntilTheEndDev/$repo/contents/$filePath?access_token=" + getGithubToken(project)
                        )
