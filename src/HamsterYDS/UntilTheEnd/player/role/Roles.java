...
public enum Roles {
    DEFAULT("鏈�夋嫨", 0, 200, 20, 1.0),
    WILSON("濞佸皵閫�", 0, 200, 20, 1.0),
    WILLOW("钖囨礇", 0, 120, 20, 1.0),
    WOLFGANG("娌冨皵澶唸", 0, 200, 30, 1.0),
    WENDY("娓╄拏", 0, 250, 20, 0.75),
    WX78("AI->WX-78", 0, 100, 10, 1.0),
    WICKERBOTTOM("缁村厠娉㈤】", 0, 250, 20, 1.0),
    WOODIE("浼嶈开", 0, 200, 20, 1.0),
    WES("缁存柉", 0, 150, 15, 0.75),
    WIGFRID("濞佹垐鑺欑憺寰�", 0, 120, 25, 1.25),
    WEBBER("闊︿集", 0, 100, 22, 1.0),
    MAXWELL("楹﹀厠鏂煢", 0, 200, 10, 1.0);
    public String name;
    ...
    static {
        ...
        for (Roles r : values()) {
            instance.getLogger().fine("[Role] Loading settings for " + r.name());
            ...
            if (section == null) {
                instance.getLogger().fine("[Role] \tSettings for " + r.name() + " not found. use default values.");
            } else {
                ...
                instance.getLogger().fine("[Role] \t\tMaxHealth: " + r.originHealthMax);
                instance.getLogger().fine("[Role] \t\tMaxSanity: " + r.originSanMax);
